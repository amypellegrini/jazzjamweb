...
If `.sensible-harness/assessment.json` is absent, run `/assess-repo` to score this repo across 9 dimensions, surface gaps, and generate the repo-specific `onboard-me` and `setup-workspace` skills. The assessment report is also the input the BA and QA roles use to build the initial backlog.
