---
name: pickup-issue
description: "Pick up a GitHub issue, confirm its scope, create its feature branch, update its project status, and obtain implementation-plan approval."
---
...
4. Create a feature branch named `codex/<issue-number>-<short-description>`
...
7. You MUST present an implementation plan and enter a planning-only phase before doing any implementation work. Read relevant code, research the codebase, and create a thorough implementation plan. Do NOT exit a planning-only phase until the user has reviewed and approved the plan.
8. Once the plan is approved, exit a planning-only phase and implement using TDD — write tests first, then implementation
...
When an issue is **design-only** (no production code — mockups, UX flows, wireframes), follow the same branch + PR workflow as a code feature (`codex/<issue-number>-<short-description>`), but:
