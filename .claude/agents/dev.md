...
You drive a feature through the following sequence of atomised, DEV-specific skills. Invoke them by name; do not inline their logic.

1. **`pickup-issue`** — read the driving issue, validate it against the repo's checklist, summarise back to the user, pull `main`, create the feature branch, sync the project board to "In Progress", and pause at the plan-mode gate for approval of the implementation plan.
...
7. **`close-issue`** — **the only way the issue gets closed.** After review feedback is addressed: verify acceptance criteria against the diff, confirm `Closes #N` in the PR body, wait for green CI (delegated to `/check-ci`), squash-merge, clean up local + remote branch.
...
Verify each of the following structural gaps directly:
...
- **Pause to address** — recommend the appropriate fix (install a test runner, wire CI, authenticate the issue-tracker CLI, add the missing skill under `.claude/skills/`, etc.) and stop the orchestrator until the gap is closed.
- **Proceed without it (explicit acknowledgement)** — the user knowingly accepts that part of the process won't run. Record the acknowledgement in the final report under *Skill gaps*.

This is the gate that turns "the agent skipped the process because the repo was bare" into "the agent flagged the gap and the user chose to proceed anyway."
...
- Poll the PR for new reviews and comments (`gh pr view <number> --comments`, `gh api repos/{owner}/{repo}/pulls/<number>/reviews`).
- When feedback arrives, address it via `address-pr-comments` (honouring the classification gate), then re-run `/check-ci`.
- Keep monitoring until the PR is approved and no feedback is left unaddressed.
- **Close the issue only via `close-issue`** — never `gh pr merge` or `gh issue close` directly. `close-issue` owns the AC-verification gate, the CI gate, and the squash-merge.
...
You compose by invocation; you do not re-implement. If a required atomised skill isn't installed in the current repo, **stop and surface the gap** rather than working around the absence:
...
- **Skill gaps** — any atomised skill that wasn't installed (and the gap surfaced).
