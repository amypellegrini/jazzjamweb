...
- The implementation plan has already been approved via plan-approval (the `pickup-issue` skill gates this). `$tdd` does not re-ask.
- The driving issue is identifiable — either explicitly passed (e.g. `$tdd <issue> <AC>`) or the current feature branch encodes the issue reference (e.g. `codex/41-…` for a GitHub issue number, `codex/PROJ-123-…` for a Jira work-item key).

## Step 1 — pick the AC

- If an AC is passed explicitly as an argument (text or index), use it verbatim. Explicit arg always overrides inference.
- Otherwise, fetch the driving issue and pick the next AC not yet exercised by a committed test on this branch. Use the CLI matching the repo's issue tracker:
  - **GitHub** (the default): `gh issue view <number>`.
  - **Jira**: `acli jira workitem view <KEY>`.
